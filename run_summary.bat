...
rem ?? Launch Brave (Claude profile) with CDP if not already running ??????????
powershell -command "try { (New-Object Net.Sockets.TcpClient('localhost', 9222)).Close(); exit 0 } catch { exit 1 }" >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    echo Starting Brave ^(Claude profile^) with remote debugging port 9222 ...
    start "" "C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe" --remote-debugging-port=9222 --profile-directory="Claude" --no-first-run --no-default-browser-check
